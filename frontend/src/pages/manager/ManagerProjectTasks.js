  ...
  if (!project) {
    return (
      <div className="p-8">
        ...
        <Button onClick={() => navigate('/dashboard/manager/projects')}>Back to Projects</Button>
      </div>
    );
  }
    ...
    <div className="p-8" data-testid="manager-project-tasks-page">
      ...
      <div className="mb-8 bg-slate-900/50 rounded-xl border border-slate-800 p-6">
        ...
        <div className="space-y-3">
          {todayTasks.length > 0 ? (
            todayTasks.map(task => (
              <div key={task.id} data-testid={`task-${task.id}`} className="bg-slate-800/50 p-4 rounded-lg border border-slate-700">
      ...
      {/* Progress Section */}
      <div className="mb-8 bg-slate-900/50 rounded-xl border border-slate-800 p-6">
        <h2 className="text-2xl font-semibold text-white mb-4" style={{fontFamily: 'Work Sans'}}>Progress</h2>
        ...
        <p className="text-slate-400 text-sm mt-2">{completedTasks} of {totalTasks} tasks completed</p>
      </div>

      {/* All Tasks Table */}
      <div className="bg-slate-900/50 rounded-xl border border-slate-800 p-6">
        <h2 className="text-2xl font-semibold text-white mb-6" style={{fontFamily: 'Work Sans'}}>All Tasks</h2>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-slate-700">
                ...
                <th className="text-left text-slate-400 font-medium py-3 px-4">Due Date</th>
            ...
            <tbody>
              {filteredTasks.map((task) => (
                <tr key={task.id} className="border-b border-slate-800">
                  ...
                  <td className="py-4 px-4 text-slate-400 text-sm">{formatDate(task.dueDate)}</td>
